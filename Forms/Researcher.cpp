 ...
 * Copyright (C) 2018 by Admiral_Fish, bumba, and EzPzStreamz
 *
...
void Researcher::on_pushButtonGenerate32Bit_clicked()
{
        ...
        if (ui->radioButtonCommon32Bit->isChecked())
        {
            switch (ui->comboBoxRNG32Bit->currentIndex())
            {
                ...
                case 8:
                    ...
                    {
                        QMessageBox error;
                        error.setText("Please enter a search range lower then 228");
                        error.exec();
                        return;
                    }
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        {
            QMessageBox error;
            error.setText("You must check the Hex box in order to use Hex values.");
            error.exec();
            return;
        }
    }
